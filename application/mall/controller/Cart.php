...
class Cart extends Base {
    ...
    public function cart3(){
        ...
        if($address_id == '' && $_REQUEST['act'] == 'submit_order') $this->ajaxReturn(['status'=>-1,'msg'=>'请选择收货地址','result'=>'']);
    ...
    public function pre_sell_cart()
    {
        ...
        if(empty($address)){
            header("Location: ".U('mall/User/add_address',array('source'=>'pre_sell_cart','act_id'=>$act_id,'goods_num'=>$goods_num)));
            exit;
        }else{
    ...
    public function integral(){
        ...
        if(empty($address)){
            header("Location: ".U('mall/User/add_address',array('source'=>'integral','goods_id'=>$goods_id,'goods_num'=>$goods_num,'item_id'=>$item_id)));
            exit;
        }else{
            $this->assign('address',$address);
        }
